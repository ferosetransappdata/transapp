...
- name: Generate the response file for software only installation
  template: src={{ Version }}.rsp.j2  dest=/tmp/{{ Version }}.rsp

#- name: Generate response Dbca
#  template: src=dborclt2.rsp.j2 dest={{INSTALLATION_FOLDER}}/dborclt2.rsp.j2
...
#  become_user: oracle
#  command: "{{ INSTALLATION_FOLDER }}/database/runInstaller -silent -ignorePrereq -ignoreSysPrereqs -waitforcompletion -responseFile /tmp/{{ Version }}.rsp"
...
- name: Install dbca
  become_user: oracle
  command: "{{ORACLE_HOME}}/bin/dbca -silent -responseFile {{INSTALLATION_FOLDER}}/dborclt2.rsp.j2"
#
#- name: Install netca
#  become_user: oracle
